...
def get_data():
    ...
    # 이동평균 계산
    for ma in [3, 5, 9, 20, 40, 60, 120, 180]:
        df[f'MA{ma}'] = df['close'].rolling(ma).mean()

    # 평균추세선
    df['MAA'] = (df['MA3']*3 + df['MA5']*2 + df['MA9']) / 6
    df['MAA3'] = df['MAA'].rolling(3).mean()

    # 볼린저밴드
    df['MA_BB'] = df['close'].rolling(60).mean()
    df['BB_STD'] = df['close'].rolling(60).std()
    ...
    df.loc[df['Market_Trend'] == "상승장", 'Buy_Signal'] = (
        ...
        (((df['MA20'].shift(4)-df['MA40'].shift(4))/(df['MA20']-df['MA40'])) <  1)
    #    (df['close'] > df['UpperBBand'])
    )

    df.loc[df['Market_Trend'] == "상승장", 'Sell_Signal'] = (
        ((df['MA3'].shift(1) > df['MA3']) &
         (df['MA9'].shift(1) > df['MA9']) &
         (df['close'] < df['UpperBBand'])) |
        ((df['MAA'].shift(1) > df['MAA']) &
         (df['MAA'].shift(2) > df['MAA'].shift(1)))
    )
    ...
    # 하락장
    df.loc[df['Market_Trend'] == "하락장", 'Buy_Signal'] = (df['Market_Trend'] == "상승장")
    df.loc[df['Market_Trend'] == "하락장", 'Sell_Signal'] = (df['Market_Trend'] == "하락장")
...
profit_targets = {"상승장": 3.0, "횡보장": 1.5, "하락장": 0.8}
stop_losses = {"상승장": -0.5, "횡보장": -0.2, "하락장": -0.8}
trailing_gaps = {"상승장": 0.2, "횡보장": 0.2, "하락장": 0.5}  # 최고점 대비 하락폭
...
while True:
    try:
        ...
        # ✅ 상태 출력 및 알림
        log_msg = (
            f"{datetime.datetime.now()} | {market} | {market_trend} | "
            f"MA20: {latest['MA20']:.1f} MA40: {latest['MA40']:.1f} MA60: {latest['MA60']:.1f} | "
