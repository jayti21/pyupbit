...
interval = "minute3"
count = 2000
trade_amount = 10000   # ✅ 지정 금액 (원화 단위)
...
while True:
    try:
        ...
        print(f"{datetime.datetime.now()} | {market} | {latest['Market_Trend']} | 현재가: {current_price:.0f} | 포지션: {'보유중' if in_position else '대기중'}")
        send_telegram(f"{datetime.datetime.now()} | {market} | {latest['Market_Trend']} | 현재가: {current_price:.0f} | 포지션: {'보유중' if in_position else '대기중'}")
